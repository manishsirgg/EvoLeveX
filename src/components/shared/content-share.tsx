...
export function ContentShare({ title, text: shareText, url, menuHeading, triggerLabel, menuLabel, nativeShareLabel, emailSubject = title, emailBody }: ContentShareProps) {
  ...
  async function useNativeShare() {
    setError('')
    try {
      await navigator.share({ title, text: shareText?.trim() || undefined, url })
      setOpen(false)
    } catch (reason) {
  ...
  const destinations = [
    ['WHATSAPP', shareUrl('https://wa.me/', { text: combinedText }), 'WA'],
  ...
  const email = `mailto:?${new URLSearchParams({ subject: emailSubject, body: emailBody })}`

  return <div className="article-share" ref={rootRef}>
    <button ref={triggerRef} type="button" className="article-bookmark" aria-label={triggerLabel} aria-haspopup="menu" aria-expanded={open} aria-controls={menuId} onClick={() => { setOpen((value) => !value); setError('') }}>
    ...
    {open ? <div id={menuId} className="article-share-menu" role="menu" aria-label={menuLabel}>
      <p>{menuHeading}</p>
      {nativeShare ? <button type="button" role="menuitem" onClick={useNativeShare}><span className="share-mark" aria-hidden="true">↗</span>{nativeShareLabel}</button> : null}
