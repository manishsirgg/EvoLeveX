...
export function ArticleLike({ articleId, articlePath, authenticated, initiallyLiked, initialLikeCount }: ArticleLikeProps) {
  ...
  async function toggleLike() {
    ...
    try {
      const response = await fetch('/api/daily/likes', {
  ...
  return (
    <div className="article-like-wrap">
      <button
        type="button"
        className="article-bookmark article-like"
        onClick={toggleLike}
        disabled={pending}
        aria-label={`${liked ? 'Unlike' : 'Like'} this article. ${likeCount} ${likeCount === 1 ? 'like' : 'likes'}.`}
        aria-pressed={liked}
        aria-busy={pending}
      >
      ...
      <span className="article-bookmark-status" role="status" aria-live="polite">{error}</span>
    </div>
  )
}
