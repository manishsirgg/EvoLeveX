...
export function MagazineArticle({ article, blocks, resources, preview = false }: { article: DailyArticle; blocks: DailyBlock[]; resources: DailyBlockResources; preview?: boolean }) {
  ...
  return <main className="article-page"><article>
    {preview && <div className="preview-banner"><strong>Admin preview</strong><span>This article may not be public.</span><Link href={`/admin/daily/${article.id}/edit`}>Return to editor</Link></div>}
    <Link href={preview ? `/admin/daily/${article.id}/edit` : '/daily'} className="article-back"><span aria-hidden="true">←</span> {preview ? 'Return to editor' : 'Back to Evo Daily'}</Link>
