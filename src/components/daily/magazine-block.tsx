...
export function MagazineBlock({ block, resources }: { block: DailyBlock; resources: DailyBlockResources }) {
  ...
  if (block.block_type === 'cta') {
    ...
    return <aside className="magazine-cta"><div>{block.heading && <h3>{block.heading}</h3>}<Copy value={block.body} /></div>{external ? <a className="magazine-action" href={url} target="_blank" rel="noopener noreferrer">{block.button_label || 'Learn more'} ↗</a> : <Link className="magazine-action" href={url}>{block.button_label || 'Learn more'} →</Link>}</aside>
  }
  return null
}
