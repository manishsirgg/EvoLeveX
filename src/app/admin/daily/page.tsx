    ...
    <section aria-labelledby="daily-admin-title">
      ...
      {!hasError && articles.length === 0 ? (
        <div className="mt-7 grid min-h-72 place-items-center border border-dashed border-white/15 bg-zinc-900/30 px-6 py-14 text-center">
          <div><p className="text-xs font-semibold uppercase tracking-[0.18em] text-amber-300">{status === 'all' ? 'Your newsroom starts here' : `No ${status} articles`}</p><h2 className="mt-3 text-2xl font-semibold">{status === 'all' ? 'Create the first Evo Daily article.' : 'Nothing matches this filter.'}</h2><p className="mx-auto mt-3 max-w-lg leading-7 text-zinc-400">{status === 'all' ? 'The publishing editor arrives in Stage 2B. The protected entry point is ready.' : 'Choose another status or begin a new article.'}</p><Link href="/admin/daily/new" className="mt-6 inline-flex bg-amber-300 px-5 py-3 text-sm font-bold text-zinc-950 hover:bg-amber-200">New Article</Link></div>
        </div>
      ) : null}
