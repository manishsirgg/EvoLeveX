  ...
  return (
    <section aria-labelledby="daily-admin-title">
      ...
      <nav aria-label="Filter articles by status" className="admin-filter-nav mt-8 flex gap-2 overflow-x-auto border-b border-white/10 pb-4">
        {filters.map((filter) => <Link key={filter.value} href={filter.value === 'all' ? '/admin/daily' : `/admin/daily?status=${filter.value}`} aria-current={status === filter.value ? 'page' : undefined} className="min-w-max px-4 py-2 text-xs uppercase tracking-wider">{filter.label}</Link>)}
      </nav>
