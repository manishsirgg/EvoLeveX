...
function EditorialImageManager({ imageUrl }: { imageUrl: string | null }) {
  ...
  return <div className="md:col-span-2 border border-white/10 bg-black/20 p-4">
    ...
    <div className="mt-3 flex flex-wrap gap-2">
      <button type="button" onClick={() => setIntent('upload')} className="button-secondary px-3 py-2 text-sm font-semibold">{imageUrl ? 'Replace image' : 'Upload image'}</button>
      {imageUrl && <button type="button" onClick={() => { setIntent('keep'); setPreview(imageUrl) }} className="button-secondary px-3 py-2 text-sm">Keep current</button>}
      <button type="button" onClick={() => setIntent('url')} className="button-secondary px-3 py-2 text-sm">Use image URL instead</button>
      {(imageUrl || preview) && <button type="button" onClick={() => setIntent('remove')} className="button-danger px-3 py-2 text-sm">Remove image</button>}
    </div>
...
function BlockForm({ articleId, block, data, content, onCancel }: { articleId: string; block?: DailyBlock; data: BlockComposerData; content: string; onCancel(): void }) {
  ...
  return <form action={action} className="mt-5 border border-amber-300/20 bg-black/20 p-4 sm:p-5">
    ...
    <div className="mt-4 grid gap-4 md:grid-cols-2">
      ...
      {fields.includes('external_url') && <label className={label}>Destination URL<input name="external_url" type={type === 'affiliate' ? 'url' : 'text'} required={type === 'affiliate'} defaultValue={model.external_url ?? ''} className={input} placeholder={type === 'cta' ? '/internal or https://…' : 'https://…'} /></label>}
    ...
    <div className="mt-4 flex flex-wrap items-end gap-4"><label className={label}>Order at position<input name="sort_order" type="number" step="1" defaultValue={model.sort_order} className={`${input} w-32`} /></label><label className="flex items-center gap-2 pb-2 text-sm text-zinc-300"><input name="is_active" type="checkbox" defaultChecked={model.is_active} className="accent-amber-300" /> Active</label><button className="button-primary px-4 py-2.5 text-sm">Save block</button><button type="button" onClick={onCancel} className="px-3 py-2 text-sm text-zinc-400">Cancel</button></div>
  </form>
}
