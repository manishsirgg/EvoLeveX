...
function BlockForm({ articleId, block, data, content, onCancel }: { articleId: string; block?: DailyBlock; data: BlockComposerData; content: string; onCancel(): void }) {
  ...
  return <form action={action} className="mt-5 border border-amber-300/20 bg-black/20 p-4 sm:p-5">
    ...
    <div className="mt-4 flex flex-wrap items-end gap-4"><label className={label}>Order at position<input name="sort_order" type="number" step="1" defaultValue={model.sort_order} className={`${input} w-32`} /></label><label className="flex items-center gap-2 pb-2 text-sm text-zinc-300"><input name="is_active" type="checkbox" defaultChecked={model.is_active} className="accent-amber-300" /> Active</label><button className="primary-action px-4 py-2.5 text-sm">Save block</button><button type="button" onClick={onCancel} className="px-3 py-2 text-sm text-zinc-400">Cancel</button></div>
  </form>
}
