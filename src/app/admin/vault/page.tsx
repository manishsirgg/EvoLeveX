...
export default async function AdminVaultPage() {
  ...
  return <section><div className="flex flex-wrap items-end justify-between gap-5"><div><p className="text-xs font-semibold uppercase tracking-[.2em] text-amber-300">Catalog</p><h1 className="mt-3 text-3xl font-semibold sm:text-4xl">Evo Vault</h1><p className="mt-3 max-w-2xl text-zinc-400">Manage the books and courses that form the Vault catalog.</p></div><div className="flex flex-wrap gap-3"><Link href="/admin/vault/categories" className="button-secondary px-5 py-3 text-sm font-bold">Manage categories</Link><Link href="/admin/vault/products" className="button-secondary px-5 py-3 text-sm font-bold">Manage products</Link><Link href="/admin/vault/products/new" className="button-primary px-5 py-3 text-sm">Add product</Link></div></div>
