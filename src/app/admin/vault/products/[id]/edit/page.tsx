...
export default async function EditVaultProductPage({ params, searchParams }: { params: Promise<{id:string}>; searchParams: Promise<{success?:string}> }) {
  ...
  return <ProductEditor {...record} categories={categoryOptions.categories} categoriesError={categoryOptions.hasError} instructors={options.instructors} instructorsError={options.hasError} feedback={query.success} />
}
