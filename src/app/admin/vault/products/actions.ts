...
const keys = ['kind','name','slug','short_description','description','product_mode','price','cover_image_url','sort_order','seo_title','seo_description','author_name','isbn','page_count','physical_weight_g','preview_text','instructor_id','subtitle','level','duration_minutes','preview_video_url']
...
async function mutate(id: string | null, data: FormData): Promise<VaultActionState> {
  ...
  if (id) {
    ...
    const existing = await supabase.from('evo_vault_products').select('kind').eq('id', id).maybeSingle()
  ...
  const parent = { kind, name, slug, short_description: nullable(text(data,'short_description')), description: nullable(text(data,'description')), product_mode: mode, price: Number(priceRaw), currency: 'INR', cover_image_url: nullable(text(data,'cover_image_url')), is_active: data.get('is_active') === 'on', is_featured: data.get('is_featured') === 'on', sort_order: Number(sortRaw), seo_title: nullable(text(data,'seo_title')), seo_description: nullable(text(data,'seo_description')) }
