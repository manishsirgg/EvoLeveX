export const PUBLIC_IMAGE_BUCKET = 'evo-public'
export const PUBLIC_IMAGE_MAX_BYTES = 5 * 1024 * 1024
export const PUBLIC_IMAGE_ACCEPT = 'image/jpeg,image/png,image/webp'
...
export function validatedImageExtension(file: File) {
...
export function ownedPublicImagePath(urlValue: string | null, expectedPrefix: string) {
  if (!urlValue) return null
  const base = process.env.NEXT_PUBLIC_SUPABASE_URL
  if (!base) return null
  try {
    const url = new URL(urlValue)
    const expected = new URL(`/storage/v1/object/public/${PUBLIC_IMAGE_BUCKET}/`, base)
    ...
    return /^[0-9a-f]{8}-[0-9a-f]{4}-4[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}\.(?:jpg|png|webp)$/i.test(filename) ? path : null
  } catch {
    return null
  }
}
